...
pub fn get_prime(size: u32) -> Integer {
    ...
    while !miller_rabin(&i) {
        i.assign(Integer::random_bits(size, &mut rand));
    }
    i
}

pub fn get_prime_max(max: &Integer) -> Integer {
    let mut rand = RandState::new();
    let seed = Integer::from(SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_micros());
    rand.seed(&seed);
    let mut i = Integer::from(max.random_below_ref(&mut rand));
    while !miller_rabin(&i) {
        i.assign(max.random_below_ref(&mut rand));
    }
    i
}
...
pub fn miller_rabin(n: &Integer) -> bool {
    ...
    for a in primes {
        ...
        if utils::check_composite(&n, &a, &d, s) {
            return false;
        }
    }
